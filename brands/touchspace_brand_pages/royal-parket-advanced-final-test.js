(function () {
  ...
  var raw = window.RoyalParketAdvancedFinalTestData;
  ...
  function saveResult(state) {
    ...
    window.dispatchEvent(new CustomEvent("touchspace-course-progress-change",{detail:{course:"royal-parket-advanced",final:true}}));
  }
  ...
  function renderLocked() {
    stage.innerHTML = '<article class="lesson-slide is-active"><span class="lesson-kicker">Итог курса</span><h2 class="lesson-title">Итоговый тест Royal Parket Advanced</h2><p class="lesson-lead">Финальная проверка открывается после успешного прохождения итоговых тестов всех семи модулей.</p><div class="blocks"><div class="block coswick-callout">Сейчас тест закрыт. Вернитесь в академию и завершите оставшиеся модули.</div></div><div class="lesson-actions"><a class="btn" href="royal-parket-advanced-academy.html">К модулям академии</a></div></article>';
  }
  function renderTest() {
  ...
  function check() {
    ...
    result.textContent = score + "%. " + (passed ? "Курс Royal Parket Advanced успешно пройден." : "Проходной результат не достигнут. Проверьте пояснения и повторите попытку.");
