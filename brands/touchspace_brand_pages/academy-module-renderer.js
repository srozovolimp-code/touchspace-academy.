(function () {
  ...
  function renderModule() {
    ...
    function renderStage(stateValue) {
      if (current === pages.length) {
        ...
        return '<article class="academy-slide lesson-slide is-active"><span class="academy-kicker lesson-kicker">Итог</span><h2 class="academy-title lesson-title">' + esc(finalTest.название || 'Итоговый тест модуля') + '</h2><p class="academy-lead lesson-lead">' + esc(finalTest.описание || 'Проверка по материалам модуля.') + '</p><div class="academy-actions lesson-actions"><div class="academy-actions__row lesson-actions__row"><button class="btn secondary" type="button" data-prev>Назад</button><button class="btn" type="button" data-open-test="' + pages.length + '">' + (finalPassed(stateValue) ? 'Повторить итоговый тест' : 'Пройти итоговый тест') + '</button><a class="btn" href="' + esc(nextUrl) + '"' + (canContinue ? '' : ' aria-disabled="true" tabindex="-1"') + '>' + nextLabel + '</a><span class="result ' + (canContinue ? 'good' : '') + '">' + (canContinue ? 'Модуль пройден.' : 'Для перехода нужен результат не ниже ' + threshold(finalTest, 80) + '%.') + '</span></div></div></article>';
      }
  ...
  function renderFinalTest() {
    ...
    var academyUrl = academyHref(manifest);
    var academyName = academyTitle(manifest);
    root.innerHTML = '<div class="academy-topline"><nav class="breadcrumb" aria-label="Хлебные крошки"><a href="../../brands.html">Бренды</a><span>/</span><a href="' + academyUrl + '">' + academyName + '</a><span>/</span><span>' + esc(test.название) + '</span></nav><a class="back-link" href="' + academyUrl + '">Назад в академию</a></div><section class="academy-stage stage"><article class="academy-slide lesson-slide is-active"><span class="academy-kicker lesson-kicker">Итоговая проверка</span><h1 class="academy-title lesson-title">' + esc(test.название) + '</h1><p class="academy-lead lesson-lead">' + esc(test.описание || '') + '</p><div class="academy-actions lesson-actions"><div class="academy-actions__row lesson-actions__row"><button class="btn" type="button" data-open-final' + (canOpen ? '' : ' disabled') + '>Открыть тест</button><span class="result" data-final-result aria-live="polite" role="status">' + (canOpen ? 'Для прохождения нужно набрать не менее ' + threshold(test, 85) + '%.' : 'Итоговый тест откроется после прохождения всех модулей.') + '</span></div></div></article></section><div class="quiz-modal" data-quiz-modal aria-hidden="true"><div class="quiz-modal__panel" role="dialog" aria-modal="true" aria-labelledby="quiz-title"><div class="quiz-modal__head"><div><span>Итоговая проверка</span><h3 id="quiz-title">' + esc(test.название) + '</h3></div><button class="icon-btn" type="button" data-close-quiz aria-label="Закрыть тест">×</button></div><div class="quiz-modal__body"><div data-quiz-content>' + (test.вопросы || []).map(function (question, index) { return questionHtml(question, 0, index); }).join("") + '</div><div class="feedback" data-feedback></div></div><div class="quiz-modal__foot"><button class="btn secondary" type="button" data-close-quiz>Вернуться</button><button class="btn" type="button" data-check-final>Проверить тест</button><span class="result" data-modal-result aria-live="polite" role="status"></span></div></div></div>';
