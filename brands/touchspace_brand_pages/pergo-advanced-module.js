(function () {
  var config = window.PergoAdvancedModuleConfig;
  if (!config) return;
  ...
  function completedCount(state) {
    return pages.reduce(function (total, page) { return total + (pagePassed(state, pageId(page)) ? 1 : 0); }, 0) + (finalPassed(state) ? 1 : 0);
  }
  ...
  function normalizeQuestion(question) {
    ...
    return {
      ...
      q: humanText(question.текст_вопроса || ''),
      ...
      exp: humanText(question.пояснение || '')
    };
  }
  ...
  function renderBlock(block) {
    ...
    if (type === 'карточки') {
      ...
      body = '<div class="coswick-timeline">' + (content.события || []).map(function (event) { return '<article><b>' + safe(event.год) + '</b><p>' + safe(event.событие) + '</p></article>'; }).join('') + '</div>';
      ...
      body = '<div class="coswick-examples">' + (content.примеры || []).map(function (example) {
        var parts = Object.keys(example).map(function (key) { return '<p><b>' + esc(humanLabel(key)) + ':</b> ' + safe(example[key]) + '</p>'; }).join('');
  ...
  function renderTabs(state, currentIndex) {
    ...
    tabs += '<button class="lesson-tab' + (currentIndex === pages.length ? ' is-active' : '') + (finalDone ? ' is-done' : '') + (finalLocked ? ' is-locked' : '') + '" type="button" data-go="' + pages.length + '"' + (finalLocked ? ' disabled' : '') + '><span class="tab-num">★</span><span><span class="tab-title">' + safe(finalTest.название || 'Итоговый тест модуля') + '</span><span class="tab-meta">' + (finalLocked ? 'закрыт' : (finalDone ? 'пройдено' : (canBypassTests() ? 'доступен без теста' : 'доступен'))) + '</span></span><span class="tab-check">' + (finalDone ? '✓' : '') + '</span></button>';
  ...
  function renderStageContent(index, state) {
    if (index === pages.length) {
      ...
      return '<article class="lesson-slide is-active" data-rendered-page="' + index + '"><span class="lesson-kicker">Итог</span><h2 class="lesson-title">' + safe(finalTest.название || 'Итоговый тест модуля') + '</h2><p class="lesson-lead">' + safe(finalTest.описание || 'Проверка по материалам модуля.') + '</p><div class="blocks"><div class="block coswick-callout">' + (bypass ? 'Для администратора и поставщика следующий шаг доступен без прохождения теста.' : 'Следующий шаг откроется после результата итогового теста не ниже ' + threshold(finalTest, 80) + '%.') + '</div></div><div class="lesson-actions"><div class="lesson-actions__row"><button class="btn secondary" type="button" data-course-prev>Назад</button><button class="btn" type="button" data-open-test="' + pages.length + '">' + (state.finalTest.passed ? 'Повторить итоговый тест' : 'Пройти итоговый тест') + '</button><a class="btn" href="' + esc(nextModuleUrl || 'pergo-advanced-academy.html') + '"' + (canContinue ? '' : ' aria-disabled="true" tabindex="-1"') + '>' + nextModuleLabel + '</a><span class="result ' + (canContinue ? 'good' : '') + '">' + (state.finalTest.passed ? 'Модуль пройден.' : (bypass ? 'Доступ открыт для вашей роли.' : 'Для перехода нужен итоговый результат не ниже ' + threshold(finalTest, 80) + '%.')) + '</span></div></div></article>';
    }
    var page = pageAt(index);
    var passed = pagePassed(state, pageId(page));
