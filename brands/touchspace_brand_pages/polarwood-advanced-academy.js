(function () {
  ...
  var academy = {
    name: "Polarwood",
    category: "Паркетная доска из дуба и ясеня",
    logo: manifest.логотип,
    summary: "Последовательный курс по продуктовой системе Polarwood: конструкция, породы и сортировки, коллекции, цвета, подбор, монтаж, эксплуатация и гарантия."
  };
  var pageUrl = "brands/touchspace_brand_pages/polarwood-advanced-academy.html";
  document.title = "Академия Polarwood Advanced | TouchSpace Академия";
  ...
  var icons = ["../../icons/5.png","../../icons/7.png","../../icons/8.png","../../icons/9.png","../../icons/1.png","../../icons/2.png","../../icons/3.png"];
  ...
  function courseCard(record, index) {
    ...
    var favorite = favoriteButton("academy-polarwood-advanced-course-" + id, title, "Курс академии", fullUrl, desc);
    var schedule = canPlanAccount() && unlocked ? planButton("academy-polarwood-advanced-" + id, title, id) : "";
  ...
  function finalCard() {
    ...
    var favorite = favoriteButton("academy-polarwood-advanced-final-test", title, "Тест академии", "brands/touchspace_brand_pages/" + record.url, desc);
    return '<article class="course-card ' + (unlocked ? 'is-link' : 'is-locked') + '"' + (unlocked ? ' data-href="' + esc(record.url) + '"' : '') + '>' +
  ...
  root.innerHTML = '<div class="topline">' +
    '<nav class="breadcrumb" aria-label="Хлебные крошки"><a href="../../brands.html">Бренды</a><span>/</span><span>Академия Polarwood Advanced</span></nav>' +
    '<a class="back-link" href="../../brands.html">Назад к каталогу брендов</a></div>' +
    '<section class="academy-hero"><div><span class="eyebrow">Академия бренда</span><h1>Академия Polarwood</h1><p>' + esc(academy.summary) + '</p>' +
    '<div class="academy-actions"><button class="brand-hero-favorite academy-favorite" type="button" aria-label="Добавить Академию Polarwood в избранное" aria-pressed="false" data-favorite-toggle data-favorite-id="academy-polarwood-advanced" data-favorite-title="Академия Polarwood Advanced" data-favorite-type="Академия бренда" data-favorite-url="' + esc(pageUrl) + '" data-favorite-description="' + esc(academy.summary) + '"><img class="favorite-icon" src="../../icons/favorites.svg" alt="" aria-hidden="true"></button>' +
    (canPlanAccount() ? '<button class="academy-plan-btn" type="button" data-plan-academy="polarwood-advanced" data-plan-id="academy-polarwood-advanced" data-plan-title="Академия Polarwood Advanced" aria-label="Запланировать Академию Polarwood" aria-pressed="false"><img class="calendar-icon" src="../../icons/calendar.svg" alt="" aria-hidden="true"></button>' : '') +
    '</div></div><aside class="brand-box"><div class="brand-logo"><img src="' + esc(academy.logo) + '" alt="Polarwood"></div><strong>Polarwood</strong><span>' + esc(academy.category) + '</span></aside></section>' +
  ...
  document.addEventListener("click", function (event) {
    ...
    if (planAcademy) {
      ...
      openPlanner({id:"academy-polarwood-advanced",type:"academy",brand:"Polarwood",lesson:"Академия",title:"Академия Polarwood Advanced",duration:329,url:pageUrl,heading:"Запланировать академию",subtitle:"Polarwood · вся академия бренда"});
      return;
    }
    if (planCourse) {
      event.preventDefault(); event.stopPropagation();
      ...
      if (record) openPlanner({id:planCourse.getAttribute("data-plan-id"),type:"course",brand:"Polarwood",lesson:record.название,title:record.название,duration:record.продолжительность_минут,url:"brands/touchspace_brand_pages/" + record.url,heading:"Запланировать курс",subtitle:"Polarwood · " + record.название});
      return;
    }
